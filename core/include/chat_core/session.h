...
class chat_session {
 public:
  ...
  ~chat_session();
  // NEED TO DELETE COPY CONSTRUCTOR AND MAKE A MOVE CONSTRUCTOR, OTHERWISE WE
  // WOULD NOT BE ABLE TO PUSH BACK INTO THE session_manager VECTOR OF SESSIONS
  chat_session(chat_session&) = delete;
  chat_session(chat_session&&) noexcept;

  void send_msg(const std::string& msg);
  void do_send_msg(const std::string& msg);
  void listen_msg(const msg_handler_t& handler);
  std::string get_local_address();
  std::string get_remote_address();

 private:
  void save_received(const boost::system::error_code& error,
