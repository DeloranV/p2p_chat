...
protected:
      ...
      server_msg_("Sent by server")
  {
    server_thread_ = boost::thread([this] {
      server_.await_connection([this](ssl_socket socket) {
        server_session_ = std::make_unique<session::chat_session>(server_io_context_, std::move(socket));
        server_session_->listen_msg([this](std::string remote_endpoint, const util::ChatError err, messages::ChatMessage msg){
  ...
  std::unique_ptr<session::chat_session> server_session_;
  std::unique_ptr<session::chat_session> client_session_;
...
TEST_F(ChatSessionIntegration, MessageSendReceive) {
  ...
  client_session_ = std::make_unique<session::chat_session>(client_io_context_, std::move(client_socket));
  client_session_->listen_msg([&client_received_msg](std::string remote_endpoint, const util::ChatError err, messages::ChatMessage msg) {
    auto msg_content = msg.msg_contents();
    client_received_msg.set_value(msg_content);
  });

  client_thread_ = boost::thread([this] {
    client_io_context_.run();
  });

  std::string client_msg_ = "Sent by client";
  client_session_->send_msg(client_msg_);

  ASSERT_NE(client_f.wait_for(std::chrono::seconds(2)), std::future_status::timeout);
  ASSERT_NE(server_f.wait_for(std::chrono::seconds(2)), std::future_status::timeout);
